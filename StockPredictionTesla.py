...
for i in range(3):
	...
	print("CONF MATRIX on verification data: \n", cm)
	CL_report = classification_report(Y_valid, Y_predicted)
	Y_predicted = np.where(models[i].predict_proba(X_train)[:,1] > 0.5, 1, 0)
	cm = confusion_matrix(Y_train, Y_predicted)
	print("CONF MATRIX on training data: \n", cm)
	print("classification_report on verification data: \n", CL_report)
	print()
